...
uint8_t steady_state(Event event, uint16_t arg) {
	...
	static uint8_t level_before_stacked = 0;
	#if  defined(USE_LVP) && defined(USE_LOW_BATT_WARN)
	static uint8_t arrived_via_1H_from_OFF = 0;
	#endif
    ...
    if (event == EV_enter_state) {
        ...
        ramp_direction = 1;
		#if  defined(USE_LVP) && defined(USE_LOW_BATT_WARN)
		if(current_event == EV_click1_hold){
			arrived_via_1H_from_OFF = 1;
		}
		else
			voltage_check();
		#endif
	...
	// If we just got back from config mode or battery warning,
	// go back to previous level
	else if (event == EV_reenter_state) {
		arg = level_before_stacked;
		set_level_and_therm_target(arg);
		return MISCHIEF_MANAGED;
	}
	// config or battery warning state has just been pushed on top of us
	else if (event == EV_stacked_state) {
		//so save the current brightness level
		level_before_stacked = actual_level;
		return MISCHIEF_MANAGED;
	}
    #if (B_TIMING_OFF == B_RELEASE_T)
    ...
    // reverse ramp direction on hold release
        ...
        #endif
		#if  defined(USE_LVP) && defined(USE_LOW_BATT_WARN)
		if(arrived_via_1H_from_OFF)
		{
			arrived_via_1H_from_OFF = 0;
			voltage_check();
		}
		#endif
        return MISCHIEF_MANAGED;
        ...
        if (sunset_timer) {
			...
			#endif //USE_THERMAL_REGULATION
            #else
            set_level_and_therm_target(dimmed_level);
            #endif //USE_SET_LEVEL_GRADUALLY
        }
        #endif  // ifdef USE_SUNSET_TIMER
    ...
    // 3H: momentary turbo (on lights with no tint ramping)
        ...
        set_level_and_therm_target(memorized_level);
		// Override reenter brightness level in case of an aborted low voltage notification.
		// In that case this condition will activate
		// (due to the evt being fw'd by low_batt_warning_state) BEFORE the reenter condition.
		level_before_stacked = memorized_level;
        return MISCHIEF_MANAGED;
    }
