...
static inline void ADC_voltage_handler() {
    ...
    // if low, callback EV_voltage_warn / EV_voltage_low
	...
	//Start init timer as soon as standby is exited
	//Reload init timer during standby
	if(go_to_standby){
		voltage_init_delay = T_LOW_BATT_WARN_INIT_DELAY_START;
	}
	else if (voltage_init_delay) {
		voltage_init_delay --;
	}
	
	//decrement the low battery warning timer
	if (voltage_cooldown_timer) {
		if(go_to_standby){
			if(voltage_cooldown_timer < T_LOW_BATT_WARN_COOLDOWN_FACTOR_STANDBY)
				voltage_cooldown_timer = 0;
			else
				voltage_cooldown_timer -= T_LOW_BATT_WARN_COOLDOWN_FACTOR_STANDBY;
		}
		else {
			voltage_cooldown_timer --;
		}
	}
	//If the timer reached 0, and the voltage measurably decreased, while not in standby
	//emit a warning event
	else if((voltage < last_voltage) && !voltage_init_delay)/* test: if(!voltage_init_delay)*/ {
		#ifdef DUAL_VOLTAGE_FLOOR
		if ((((voltage < VOLTAGE_WARN) && (voltage > DUAL_VOLTAGE_FLOOR)) || (voltage < DUAL_VOLTAGE_WARN_LOW))){
		#else
		if (voltage < VOLTAGE_WARN) {
		#endif
		//Reset the timer only when the notification state is actually activated
		//i.e. in main.c\low_battery_notification() and not here
			//voltage_cooldown_timer=T_LOW_BATT_WARN_COOLDOWN_START;
			emit(EV_voltage_warn, 0);
		}
	}
