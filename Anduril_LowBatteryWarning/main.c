...
//#define CONFIGFILE cfg-sofirn-IF25A-grandma-edition.h
...
#include "low-batt-warning-mode.h"
#endif
...
#if  defined(USE_LVP) && defined(USE_LOW_BATT_WARN)
#include "low-batt-warning-mode.c"
#endif
...
void loop() {
	...
	#if  defined(USE_LVP) && defined(USE_LOW_BATT_WARN)
	else if (state == low_batt_warning_state) {
		low_batt_warning_state_iter();
	}
	#endif
...
void low_battery_notification(){
	...
	#endif
	#ifdef USE_MOMENTARY_MODE
	(state == momentary_state && momentary_active)	||
	#endif
	#ifdef USE_LOCKOUT_MODE
	(state == lockout_state	  && actual_level > 0)	||
	#endif
	(state == off_state		  && actual_level > 0))	{
		voltage_cooldown_timer=T_LOW_BATT_WARN_COOLDOWN_START;
		push_state(low_batt_warning_state, (uint16_t)state);
	}
}
