...
uint8_t load_eeprom() {
    ...
    for(EEP_OFFSET_T i=0; i<EEPROM_BYTES; i++) {
...
uint8_t load_eeprom_wl() {
    ...
    if (found) {
        // load the actual data
        for(EEP_OFFSET_T i=0; i<EEPROM_WL_BYTES; i++) {
...
void save_eeprom_wl() {
    ...
    for (EEP_OFFSET_T i = 0; i < EEPROM_WL_BYTES+1; i ++) {
    ...
    offset ++;
    // user data
    for(EEP_OFFSET_T i=0; i<EEPROM_WL_BYTES; i++, offset++) {
        eeprom_update_byte(offset, eeprom_wl[i]);
    }
    // FIXED: write the marker last, to signal completed transaction
